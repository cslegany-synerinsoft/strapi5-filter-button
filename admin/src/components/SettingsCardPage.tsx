...
const SettingsCardPage = (props: SettingsCardPageProps) => {
    const { isLoading, isSaving, settings, onAddCard, onRemoveCard, onSubmit, updateItem, updateManyToMany, updateSortOrderChange } = props;
    const { formatMessage } = useIntl();

    const checkFormErrors = () => {
        return settings.findIndex(x => !x.from || !x.to || !x.customFieldName || !x.filterBy || !x.sortBy) !== -1;
    }
